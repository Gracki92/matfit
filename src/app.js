...
function isValidGtin(value) {
  ...
  var sum = 0;
  var payload = code.slice(0, -1);
  for (var i = payload.length - 1, weight = 3; i >= 0; i--, weight = weight === 3 ? 1 : 3) {
...
function mergeProductCatalog(storedProducts) {
  var baseIds = new Set(PRODUCTS_DEFAULT.map(function (product) {
    return product.id;
  }));
  var seenIds = new Set();
  var userProducts = (Array.isArray(storedProducts) ? storedProducts : []).filter(function (product) {
    if (!product || !product.id || baseIds.has(product.id) || seenIds.has(product.id)) return false;
    seenIds.add(product.id);
    return true;
  });
  return userProducts.concat(PRODUCTS_DEFAULT);
}
function onlyUserProducts(products) {
  var baseIds = new Set(PRODUCTS_DEFAULT.map(function (product) {
    return product.id;
  }));
  return (Array.isArray(products) ? products : []).filter(function (product) {
    return product && product.id && !baseIds.has(product.id);
  });
}
...
function plannedMealCopyKey(meal) {
  var items = (meal && Array.isArray(meal.items) ? meal.items : []).map(function (item) {
    return String(item.productId || "") + ":" + String(rGram(parseFloat(item.grams) || 0));
  }).join("|");
  return [meal && meal.type || "", meal && (meal.sourceId || meal.name) || "", meal && meal.mealTime || "", meal && meal.grams || "", meal && meal.portions || "", items].join("::");
}
function clonePlannedMeal(meal, overrides) {
  return _objectSpread(_objectSpread(_objectSpread({}, meal), overrides || {}), {}, {
    id: Date.now() + Math.random(),
    items: (meal && Array.isArray(meal.items) ? meal.items : []).map(function (item) {
...
function App() {
  ...
  var products = useMemo(function () {
    return mergeProductCatalog(storedProducts);
  }, [storedProducts]);
  function setProducts(value) {
    setStoredProducts(function (previousStored) {
      var currentCatalog = mergeProductCatalog(previousStored);
      var nextCatalog = typeof value === "function" ? value(currentCatalog) : value;
      return onlyUserProducts(nextCatalog);
    });
  }
  ...
  function toggleProductFavorite(id) {
    setProductFavorites(function (prev) {
      var ids = Array.isArray(prev) ? prev : [];
      return ids.includes(id) ? ids.filter(function (value) {
        return value !== id;
      }) : [id].concat(_toConsumableArray(ids));
    });
  }
  ...
  function rememberProductUse(id, grams) {
    ...
    setRecentProducts(function (prev) {
      return [id].concat(_toConsumableArray((Array.isArray(prev) ? prev : []).filter(function (value) {
        return value !== id;
      }))).slice(0, 8);
    });
  ...
  function lookupBarcode(value) {
    ...
    var existing = products.find(function (product) {
      return normalizeBarcode(product.ean) === ean;
    });
    if (existing) {
  ...
  function acceptScanResult() {
    ...
    var scanBrand = String(scanResult.brand || "").trim().toLowerCase();
    var dup = products.find(function (p) {
      if (scanEan) return normalizeBarcode(p.ean) === scanEan;
      return String(p.name || "").toLowerCase() === String(scanResult.name || "").toLowerCase() && String(p.brand || "").trim().toLowerCase() === scanBrand;
    });
    if (dup) {
  ...
  function saveProd() {
    ...
    if (normalizedEan && products.some(function (p) {
      return normalizeBarcode(p.ean) === normalizedEan;
    })) {
  ...
  function applyBackup(mode) {
    ...
    if (mode === "replace") {
      ...
      if (present.products) setStoredProducts(function (prev) {
        return mergeBackupRecords(onlyUserProducts(prev), incoming.products);
      });
  ...
  var productSearchNeedle = (prodSearch || "").trim().toLowerCase();
  var filtProd = products.filter(function (p) {
    var categoryMatch = productCategory === "all" || productCategory === "own" && p.custom || productCategory === "favorite" && safeProductFavorites.includes(p.id) || p.category === productCategory;
    if (!categoryMatch) return false;
    if (!productSearchNeedle) return true;
    var categoryLabel = (PRODUCT_CATEGORIES.find(function (category) {
      return category.key === p.category;
    }) || {}).label || "";
    return [p.name, p.brand, p.state, categoryLabel].some(function (value) {
      return String(value || "").toLowerCase().includes(productSearchNeedle);
    });
  });
  var userProductCount = products.filter(function (product) {
    return product.custom;
  }).length;
  var baseProductCount = products.length - userProductCount;
